...
{
    EV_S << "Time spent in IPv4NetworkConfigurator::" << name << ": " << ((double)duration / CLOCKS_PER_SEC) << "s" << endl;
}
    ...
    for (int i = 0; i < (int)routeElements.size(); i++)
    {
        ...
        try
        {
            ...
            if (!isEmpty(destinationAttr) && strcmp(destinationAttr, "*"))
                destination = AddressResolver().resolve(destinationAttr, AddressResolver::ADDR_IPv4).toIPv4();
            IPv4Address netmask;
    ...
    for (unsigned int i = 0; i < routeElements.size(); i++)
    {
        ...
        try
        {
            ...
            if (!isEmpty(sourceAttr) && strcmp(sourceAttr, "*"))
                source = AddressResolver().resolve(sourceAttr, AddressResolver::ADDR_IPv4).toIPv4();
            IPv4Address netmask;
            if (!isEmpty(netmaskAttr) && strcmp(netmaskAttr, "*"))
            {
                if (netmaskAttr[0] == '/')
        ...
        InterfaceEntry *&outIE, IPv4Address& outGateway, IPv4Topology& topology)
{
    ...
    outGateway = AddressResolver().resolve(gatewayAttr, AddressResolver::ADDR_IPv4).toIPv4();
...
void IPv4NetworkConfigurator::addStaticRoutes(IPv4Topology& topology)
{
    ...
    for (int i = 0; i < topology.getNumNodes(); i++)
        addStaticRoutes(topology, (Node *)topology.getNode(i));
}

void IPv4NetworkConfigurator::addStaticRoutes(IPv4Topology& topology, Node * sourceNode)
{
